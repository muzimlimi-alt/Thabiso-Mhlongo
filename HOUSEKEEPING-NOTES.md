...
## Post-Phase-8: project-folder cleanup (user request, 2026-09-22)

Not a Phase 8 candidate category — a direct, separate request ("clean up the project folder... move
files not used by the main project to TBC/"). `TBC/` is the user's own established destination for
this (per their own description: "stores all the temp or test files that will later be destroyed"),
already `.gitignore`'d at the repo root — so unlike every other Phase 8 move, none of this touches git
history or requires a commit; it's local disk organization only.

**Moved to `TBC/` (7 files, all previously untracked, all independently re-verified before moving —
not just recalled from memory):**
- `scratch/_debug_pdf_extract.js` → `TBC/scratch/` — zero references anywhere, same shape as the
  already-removed `scratch_screenshot_inq.js` (a root-level `scratch/` directory distinct from
  `TBC/scratch/`, apparently a second, parallel throwaway-script location).
- `HOUSEKEEPING-NOTES-DESKTOP-HM7U97M.md`, `admin-DESKTOP-HM7U97M.html`, `app-DESKTOP-HM7U97M.js` →
  `TBC/onedrive-conflict-copies/` — the OneDrive self-conflict duplicates investigated in depth earlier
  this session (confirmed byte-identical to either current `HEAD` or a specific already-committed
  commit; deletion was blocked by the auto-mode safety classifier as "Irreversible Local Destruction,"
  but a plain move isn't destructive, so this finally closes that out).
- `images/Unconfirmed 26053.crdownload`, `images/snapins-ai_36461051042180978801-scaled.jpg` →
  `TBC/images/` — a Chromium partial-download temp file and a screenshot-tool artifact, both unrelated
  to the app, both confirmed junk when first found.
- `images/banners/1787833840295-02683f15.png` → `TBC/images/` — re-investigated before moving (it had
  been left an open question earlier): a **valid** 1300×60 PNG (not corrupt, despite its suspiciously
  small 307-byte size), but zero references anywhere — not in `banners` (confirmed via fresh query),
  not in source. An orphaned upload, matching the exact profile of Batch 1's Phase 8 asset-quarantine
  candidates; moved on the same evidence standard.

**Also moved (6 more files verified identical earlier this session but never relocated before the
conversation moved on):** `lib/booking-notifications-DESKTOP-HM7U97M.js`,
`lib/direct-emails-DESKTOP-HM7U97M.js`, `lib/newsletter-scheduling-DESKTOP-HM7U97M.js`,
`lib/payment-reminders-DESKTOP-HM7U97M.js`, `routes/admin/bookings-DESKTOP-HM7U97M.js`,
`routes/admin/home-social-DESKTOP-HM7U97M.js` → `TBC/onedrive-conflict-copies/{lib,routes/admin}/` —
all confirmed byte-identical (CRLF-normalized) to their current tracked counterparts back when the
OneDrive-conflict investigation was originally done; not re-verified a second time since nothing in
those files' areas has changed since.

**Deliberately NOT moved — flagged instead of acted on:**
- `database-DESKTOP-HM7U97M.sqlite` and `sessions-DESKTOP-HM7U97M.sqlite` (+ their `-shm`/`-wal`
  files) — real data conflict copies (the database one older-but-larger than live; the sessions one
  actually *newer* than live), whose exact relationship to the live files was never fully resolved
  earlier. Moving real business/session data into a folder explicitly described as "will later be
  destroyed" is a materially different risk than moving a duplicate code file — left for the user to
  look at directly rather than acted on.
- `backups/database_backup_2026-07-18T14-44-43-168Z.sqlite` — the **only** database backup that
  exists on disk (an artifact of `scripts/backup.js`, `npm run backup`). Stale (over 2 months old) but
  not "unused by the main project" in the junk sense — recommending this stays exactly where it is,
  not moved into a disposable-by-design folder.
- `docs/`, `email-previews/`, `scripts/{backup,get-google-refresh-token,seed-banner-templates}.js`,
  `Thabiso Mhlongo Official Website.code-workspace` — all confirmed live/legitimate before being ruled
  out: `docs/` is the runtime PDF-storage fallback directory (Phase 3's own comment in `.gitignore`
  explains why it still exists in-repo; has files modified today), `email-previews/` is generated by
  `js/generate-email-previews.js` (a real dev tool), the three `scripts/` files are either an active
  `npm run backup` or documented setup utilities (referenced in `README.md`), and the `.code-workspace`
  file is the user's own IDE project file.
- `db-backups/` (root) — confirmed empty, nothing to move; git doesn't track empty directories either
  way, so there's no cleanup action available beyond noting it.
