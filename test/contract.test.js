...
module.exports = async function ({ check }) {
    ...
    const em = email();
    const id = await makeBooking(em, 300, { accept: true });
    ...
    const qId = await makeBooking(email(), 310, { accept: false }); // stays QUOTED
    ...
    const balId = await makeBooking(balEmail, 320, { accept: true });
    ...
    const paidId = await makeBooking(email(), 330, { accept: true });
