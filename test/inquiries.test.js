...
const { api, pub, one, loginAs, future, getPopiaOtpCode } = support;
...
module.exports = async function ({ check }) {
    ...
    // ── CP2: POPIA request-forget now creates a reviewable request instead of anonymizing instantly ──
    // Rewritten (see popia-erasure.test.js for full coverage of the request lifecycle this delegates
    // into): the old behavior let anyone anonymize anyone else's inquiry just by knowing their email,
    // with no review step and no audit trail. It now creates a pending popia_erasure_requests row —
    // the inquiry itself is untouched until an administrator approves and processes that request.
    ...
    // request-forget now requires the same OTP email-ownership verification as the primary
    // /erasure-requests endpoint (see popia-erasure.test.js for full OTP mechanics coverage).
    const forgetOtp = await getPopiaOtpCode(forgetEmail);
    const forgetReq = await pub('POST', '/api/public/compliance/request-forget', { email: forgetEmail, otp_code: forgetOtp });
    check('CP2: request-forget -> 200 success with a reference number', forgetReq.status === 200 && forgetReq.body.success && !!forgetReq.body.reference_number, JSON.stringify(forgetReq.body));

    const forgetRequestRow = await one('SELECT * FROM popia_erasure_requests WHERE email = ?', [forgetEmail]);
    check('CP2: request-forget created a pending, source=public erasure request (not an instant mutation)', !!forgetRequestRow && forgetRequestRow.status === 'pending' && forgetRequestRow.source === 'public', JSON.stringify(forgetRequestRow));

    const stillIntact = await one('SELECT inquiry_id, sender_email, sender_name FROM inquiries WHERE sender_email = ?', [forgetEmail]);
    check('CP2: the inquiry itself is NOT anonymized yet — awaits admin approval', !!stillIntact && stillIntact.sender_name === 'Forget Me', JSON.stringify(stillIntact));

    const forgetApprove = await api('PUT', `/api/admin/popia/requests/${forgetRequestRow.id}/approve`);
    const forgetProcess = await api('POST', `/api/admin/popia/requests/${forgetRequestRow.id}/process`);
    check('CP2: once approved+processed, the request actually anonymizes real columns', forgetApprove.status === 200 && forgetProcess.status === 200, JSON.stringify({ approve: forgetApprove.body, process: forgetProcess.body }));

    const afterForget = await one('SELECT sender_email, sender_name FROM inquiries WHERE inquiry_id = ?', [stillIntact.inquiry_id]);
    check('CP2: after processing, sender_name/sender_email are anonymized (real columns, not stale name/email)', afterForget.sender_name === 'POPIA ANONYMIZED' && afterForget.sender_email !== forgetEmail, JSON.stringify(afterForget));
